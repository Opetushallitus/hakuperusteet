...
class SessionServletSpec extends FunSuite with ScalatraSuite with ServletTestDependencies {
  ...
  val hakukausiService = Mockito.mock(classOf[HakukausiService])
  val s = new SessionServlet(config, database, oppijanTunnistus, verifier, UserValidator(countries, languages), ApplicationObjectValidator(countries, educations), emailSender, hakukausiService)
  addServlet(s, "/*")
