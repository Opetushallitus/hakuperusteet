...
class FormRedirectSpec extends FunSuite with ScalatraSuite with ServletTestDependencies with BeforeAndAfterEach {
  ...
  val appSystem = ApplicationSystem("hakuOid", Some("formUri"), true, List(), Hakukausi.s2016)
  ...
  test("it should halt on hakukausi conflict") {
    database.upsertUser(testUser)
    database.upsertPayment(Payment(None, userOid.get, new Date(1000000), "", "", "", PaymentStatus.ok, Hakukausi.k2017, None))
    val res = formRedirect.doRedirect(testUser, appObject, appSystem, "educationLevel")
    res should equal(Left(409))
  }
  ...
  test("it should redirect on valid payment") {
    database.upsertUser(testUser)
    database.upsertPayment(Payment(None, userOid.get, new Date(1000000), "", "", "", PaymentStatus.ok, Hakukausi.s2016, None))
    val res = formRedirect.doRedirect(testUser, appObject, appSystem, "educationLevel")
